...
void MainWindow::WriteInFile() //导出文件
{
    ...
    for (int i = 0; i < shop.size(); i++) { //对商店商品进行遍历
        writeIn.clear();
        writeIn = shop[i].name + " " + shop[i].sort + " " + QString::number(shop[i].price) + ' ' + QString::number(shop[i].amount) + ' ' + QString::number(shop[i].expirationDate) + ' ' + QString::number(shop[i].batchNum) + ' ';
        batch* p = shop[i].head;
        while (p->next != nullptr) {
            p = p->next;
            writeIn = writeIn + QString::number(p->birthDate.year()) + ' ' + QString::number(p->birthDate.month()) + ' ' + QString::number(p->birthDate.day()) + ' ' + QString::number(p->amount) + ' ';
        }
        //成功读取完一件商品
        writeIn += '\n';
        out << writeIn; //保存到文件
